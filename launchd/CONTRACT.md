...
2-3. **체인 게이트 컬럼 (v2, 2026-07-07 · 기본 OFF)** — schedule.tsv **4번째 컬럼** = `<선행잡>[:<타임아웃HHMM(KST)>]`. `run_timer_job.sh`(A2b)가 **락 획득 전에** 읽어, 선행 `stamps/<선행잡>.last` 가 오늘(00:00 KST) 이후 갱신됐는지(=오늘 완료) 60s 폴링 대기. 타임아웃 초과 시 경고+notify 후 **폴백 실행(스킵 아님)**. ★빈 컬럼(3컬럼 현행 포함)=게이트 없음, 현행과 100% 동일. A4 catch-up 은 3컬럼만 읽어 무영향(빈 4번째=후행 탭 제거). 설계=`designs/chain_v2_design.md`, 활성화=`playbooks/PHASE2_RUNBOOK.md 부록 W4`.
3. **notify 스크립트** — 모두 repo의 기존 `scripts/notify_sisyphe_failure.sh`를 호출만 (수정 금지).
4. **heartbeat (Phase 2 감시 — 2026-07-07 신설, 감사 §5 기반)** — GHA wrapper(run_gha_job.sh)는 잡 성공 stamp 직후 **repo 루트 `heartbeats.json`**에 `{"<잡이름>": <epoch>}` upsert(원자적) 후 그 파일만 `scripts/safe_commit_push.sh -m "heartbeat: <잡> [skip ci]"`로 push. 소비자 = GHA 잔류 워치독(check_data_freshness의 heartbeat 나이 감시 섹션 — patches/heartbeat_freshness.patch, **엔트리 없는 잡은 침묵**이라 어느 시점에 적용해도 안전). 파일 없음/파싱 실패도 침묵(이관 전 상태).
