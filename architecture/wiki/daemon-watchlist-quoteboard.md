...
- **서버 저장 상태(2026-07-20 확장)**: 관심그룹(`watchlists.json`) 외에 market.html DATA 차트 별표(`market_stars.json`, [[page-market]]가 fetch)와 기기 공통 뷰 설정(`prefs.json`, 관심종목 뷰의 섹터 선택+정렬 등)을 서버에 둔다 — 브라우저 localStorage를 서버 저장으로 승격해 기기 간 공유. POST는 최대 100KB, `/prefs`는 다른 페이지 키를 보존하는 키 단위 병합. **[[page-universe]]의 관심종목 별표도 `/prefs`의 `universe_stars_v1` 키를 공유**(2026-07-31) — quoteboard 관심그룹과 별개의 KV라 이 데몬은 저장만 하고 뷰 로직은 Universe 쪽에 있다.
