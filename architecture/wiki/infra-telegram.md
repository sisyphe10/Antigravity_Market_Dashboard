...
- **실패 사유 상세 인자(2026-08-08)**: 첫 인자(잡 이름)에 이어 **선택 2번째 인자 `DETAIL`**(실패 사유 한 줄)을 받아 메시지 끝에 `<code>` 블록으로 덧붙인다. 종전엔 잡 이름별 정형 문구뿐이라 "무엇이 왜 실패했는지"는 로그를 열어야 알 수 있었다. 본문이 x-www-form-urlencoded로 실려가므로 `& % + < >`만 제거하고 300자로 컷 — ★초기엔 `=` `#`도 함께 지워 사유 문자열이 훼손됐고(같은 날 수정), 인코딩상 무해한 두 글자는 보존한다.
- **`map-weekly` 케이스(2026-08-08)**: 일요일 22:10 crontab 잡(`weekly_map_update.sh`, 시스템 지도 주간 보정 → [[page-system-map]])의 실패를 전용 문구로 알린다. **지도가 몇 주씩 조용히 낡는 것**이 이 알림이 막는 실패 양상 — launchd 타이머가 아니라 crontab이라 stamp 신선도 감시([[daemon-daily-selfcheck]])에 잡히지 않으므로 잡이 직접 이 스크립트를 부른다.
...
- **자가치료 2단계 — 실패 자가진단(2026-07-16, `scripts/diagnose_failure.sh`)**: 기본 실패 알림을 보낸 뒤 `nohup` 백그라운드로 headless claude를 띄워 실패 잡의 로그와 repo 코드를 읽히고, 원인 진단·복구 명령·수리 제안을 🩺 후속 메시지로 보낸다.
  ...
  - 구독 쿼터 headless 잡 4종 중 하나라 **Claude 인증이 만료되면 이 진단도 함께 멈춘다** — 만료 사전 경고는 [[daemon-daily-selfcheck]](08:50)가 담당.
- 맥미니 self-check(08:50)는 **이상이 있을 때만** 발송한다(2026-07-15 설계 반전) — dead-man 감시는 외부 워치독 [[gha-daily-health-check]](11:00)가 승계. 상세는 [[daemon-daily-selfcheck]].
