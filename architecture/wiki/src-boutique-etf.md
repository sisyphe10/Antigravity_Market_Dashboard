...
- **알림**(`alert.py`, Sisyphe-Bot·`subscribers.json` 브로드캐스트, `run_boutique_etf.sh`에서 수집 직후 실행): 편입(NEW)/편출(X)/급변(+/-) 계층형 텔레그램. 신규·편출은 무조건, 급변만 매매추정액 ≥50억원 또는 종목 시총의 ≥0.1%. dedup=`alert_sent` 테이블(항목 단위 증분) → 늦게 온 운용사만 `(추가)`로 나감. 채권·현금성 파킹 보유분은 수집 단계에서 통째 제외(비율이 구조적으로 커 알림 오염).
