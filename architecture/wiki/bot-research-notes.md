...
- **재실행 안전(같은 전환의 일부)**: 날짜별 single-flight + `media/summaries/<date>.state.json` **사이드카 상태**(generated→published→completed). 입력 해시가 같으면 LLM을 다시 부르지 않고 저장된 요약을 재사용하며, 게시 단계가 분리돼 **재시도가 노션 중복 페이지를 만들지 않는다**(체인이 25분 예산 안에서 여러 백엔드를 시도하는 구조라 중간 실패·재실행이 상시 가능하다는 전제). 블로킹 LLM·노션 호출은 `asyncio.to_thread`로 빼 봇 이벤트 루프를 막지 않는다. **이미지 업로드도 같은 멱등 원칙(2026-08-19)** — 재실행은 같은 GitHub 경로로 다시 PUT 해 422가 나므로, 그 경우 기존 파일을 GET 해 `download_url`을 재사용한다(GET 응답은 PUT과 달리 파일 객체가 최상위라 `['download_url']`로 읽는다). 실패해도 `None` 반환으로 그 이미지만 빠지고 게시 자체는 진행.
- **요약 첨부 이미지 상한 40장** (`summarizer.MAX_IMAGES`, 2026-08-19 20→40 상향) — 바쁜 날 31장이 들어와 초과분이 **경고 없이 요약에서 누락**됐다. 개당 8MB·총량 32MB 가드는 그대로 유지되므로 상한은 장수 기준일 뿐이다.
