...
- **지수 구성종목 스냅샷 패스(`kr_index_constituents`, 2026-08-12 신규, `fetch_index_constituents.py`)**: KOSPI200·KOSDAQ150 구성 종목을 pykrx `get_index_portfolio_deposit_file`(지수코드 1028·2203)로 받아 적재한다. 지수 PDF는 티커만 주므로 종목명은 `kr_marcap` 최신일에서 조인하고, 정원(200·150)과 실제 종목 수가 다르면 warn만 찍고 진행한다. **`(date, index_name, ticker)` upsert라 재실행할수록 스냅샷이 이력으로 누적**되고, "현재 구성"은 `MAX(date)` 필터로 뽑는다(예시 SQL은 `build_catalog.py`에 등록). ★**스케줄 잡이 아니라 수동 실행 스크립트**(`venv/bin/python3 datalake/fetch_index_constituents.py`) — launchd 7종 어디에도 배선돼 있지 않아 편입·편출 이력은 사람이 돌린 시점에만 남는다.
