...
- **Ledger 탭 → Escape Velocity 이관(2026-08-10)**: `NAV_ITEMS`의 `ledger` 항목 URL이 `/sisyphe/dashboard.html` → **`/journal/#ledger`**([[daemon-journal-api]] Escape Velocity 웹뷰)로 바뀌었다. 짝을 이뤄 `compose_personal_view.py`의 `ACTIVE_OF`에서 `dashboard.html`이 `'ledger'` → `None`이 됐다 — 페이지는 계속 합성·서빙되지만 **nav 탭을 점유하는 소유자가 아니다**(직접 URL 전용, checklist_test.html과 같은 취급). 가계부 화면의 정본이 Sisyphe 평문 페이지에서 `~/Journal` 로컬 뷰로 옮겨간 결과([[ext-sisyphe]]).
