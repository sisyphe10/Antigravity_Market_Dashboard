...
- **DATA ★ 상단 복제 블록 폐지 + 퀵필터 눌림 상태(2026-08-22)**: 별표 행을 사이드 표 맨 위에 복제해 고정하던 블록(`cmb-pin-clone` + 구분행, 주기·Country 고정 정렬)을 걷어냈다 — **별표 모아보기는 ★ 퀵필터 하나로 일원화**. `cmbApplyPin()`은 호출부 호환을 위해 함수만 남기고 과거 복제본·구분행 **청소만** 수행하므로, 이전 세대 DOM이 남아 있어도 정리된다. 짝으로 두 ★ 퀵필터 버튼의 상태 표현이 8/17 규격(항상 채움 + active=진한 테두리·볼드)에서 **안 누름=색 테두리+색 별(빈 배경) / 누름=색 채움+볼드**로 바뀌어 눌림 여부가 한눈에 읽힌다. upd 퀵필터가 걸린 동안에는 사이드 표 헤더 ★도 에메랄드로 전환(`#cmbStarTh.upd-on`)돼 **어느 퀵필터가 켜졌는지 헤더 색으로도 드러난다**(watchlist는 기존 시안 `.on`). 다크 게시본 짝 CSS는 [[web-publish-snapshot]]에서 같이 갈아야 한다.
