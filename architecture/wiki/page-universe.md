...
- **암호화폐 3종 표시(2026-08-22)**: BTC·ETH·SOL이 `암호화폐` 섹터로 종목 리스트·섹터 표에 함께 뜬다 — 주식이 아닌 자산군이 처음 들어온 사례라 섹터 필터 선택지에도 추가된다. RSI(1M)은 지수 대신 **BTC 대비**로 계산되므로(BTC 자신은 0 부근) 다른 행과 기준이 다르다([[src-universe]]).
