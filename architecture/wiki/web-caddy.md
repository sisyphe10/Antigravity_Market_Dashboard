...
- **루트(`/`)는 2026-07-16부터 `/sisyphe/memento.html`로 302 리다이렉트** — 사용자 확정으로 AoE 기본 화면이 Memento가 됐다. **구 랜딩(`/index.html`)도 같은 곳으로 302** → 랜딩 페이지는 개인 ts.net에서 폐지(파일은 잡들이 계속 생성하나 접근 불가, [[page-index]]). 2026-07-15의 `/` → `/watchlist/` 302를 한 세대 만에 대체한 것으로, Watchlist는 `/watchlist/`와 nav 탭으로 남는다([[daemon-watchlist-quoteboard]]).
