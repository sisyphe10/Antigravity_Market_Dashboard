...
맥미니 상태를 매일 08:50 KST 텔레그램 한 줄로 요약하는 launchd 데몬(`com.antigravity.daily-selfcheck`). **정상이어도 매일 발송** — 메시지의 존재 자체가 liveness 신호(침묵=다운). 2026-07-11 컷오버로 라이브.
...
- 실제 문제(봇 다운/타이머 STALE/디스크 부족/웹 다운)만 ⚠️ 라인으로 헤더를 뒤집는다.
