...
- **롤백은 env 한 줄**: 8/18 이관분은 코드 기본값이 구경로라 켜는 쪽이 env였고(어닝봇 `EARNINGS_ANALYSIS_BACKEND`/`EARNINGS_TRANSLATE_BACKEND` 기본 `api`, 리서치봇 `RESEARCH_LLM=api`), 8/20 확산분은 반대로 **headless가 코드 기본값**이라 env가 되돌리는 쪽(`TAG_ENGINE`·`OCR_ENGINE`·`EARNINGS_SEARCH_BACKEND`·`NEWHIGH_LLM`·`FEATURED_NEWS_LLM`·`SEONYUDUO_LLM` = `api`)이다. **유료 3차 폴백은 기본 잠김**(`RESEARCH_ALLOW_PAID_FALLBACK=1`일 때만) — 기본 상태에서 크레딧 소비 0이 구조적으로 보장된다.
...
- ★**2026-08-20 전 파이프라인 확산 + 유료 전환 사전승인 규칙**: 어닝봇·리서치봇에서 검증된 어댑터를 남은 종량 호출부 6곳에 일괄 적용해 **repo의 상시 LLM 호출이 사실상 전부 구독 쿼터**로 넘어왔다 — [[src-research-tagging]] 문서 태거(`tag_docs`, `TAG_ENGINE`) · [[src-research-ocr]] 이미지 OCR(`OCR_ENGINE`, `call_multimodal` 재사용) · [[src-earnings-pipeline]] 전문 탐색 web_search(`EARNINGS_SEARCH_BACKEND`) · [[src-featured-kis]] 신고가 테마(`NEWHIGH_LLM`) · [[page-featured]] 섹터 뉴스 요약(`FEATURED_NEWS_LLM`) · [[bot-seonyuduo-exercise]] 운동기록 분류(`SEONYUDUO_LLM`). 공통 계약: **env 스위치의 코드 기본값이 `headless`이고 `api`는 롤백 경로**(8/18 이관분과 기본값 방향이 반대 — 그때는 신규 경로가 기본값이 아니었다), **유료 API 사용은 사전 승인 사항**이라 자동 폴백을 두지 않는다. API 키 가드도 `api` 분기 안으로 옮겨 headless가 키 없이 도는 것이 정상 상태가 됐다.
  - 소비처마다 **다른 계약 차이를 흡수해야 한다**: headless에는 `max_tokens` 계약이 없어(출력 통제는 프롬프트+파서, 응답은 길이 슬라이스), 배치 실패 위계는 rc(75=엔진 장애·78=정책 차단)로 표현하며, 도구가 필요한 호출(web_search)은 봉인 코어 대신 소비처가 CLI를 직접 띄우되 격리 헬퍼만 재사용한다.
- 검증 자산: `tests_headless_llm.py`가 fake-CLI 픽스처로 인증 실패·쿼터·도구 유출·타임아웃 등 계약을 고정한다(도입 시 14/14 통과).
- 소비자: [[src-earnings-pipeline]](전문 탐색 web_search)·[[timer-earnings-night-llm]](전문 번역·분석 시트) · [[bot-research-notes]](23:00 일일 요약) · [[src-research-tagging]](문서 태깅) · [[src-research-ocr]](이미지 OCR) · [[src-featured-kis]](신고가 테마) · [[page-featured]](섹터 뉴스 요약) · [[bot-seonyuduo-exercise]](운동기록 분류).
