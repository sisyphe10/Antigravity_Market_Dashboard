...
- **2026-08-17 DATA ★ 퀵필터 다크 대응 — 새 UI 컨트롤은 합성기까지 손봐야 한다**: `AOE_DARK_CSS`의 일괄 pill 규칙(`.mbtn.active` 등 = 앰버 채움)이 [[page-market]] DATA 사이드바에 새로 생긴 두 ★ 퀵필터(watchlist / update)를 같은 앰버로 뭉개 구분이 사라졌다. 합성기에 `[data-qv="star"]`=시안·`[data-qv="upd"]`=에메랄드 채움 + active는 흰 테두리 규칙을 **일괄 규칙 뒤에** 추가해 되살렸다(생성기 쪽 라이트 규격은 active 테두리가 `#222`, 다크는 `#fff`). 즉 생성기에서 새 버튼/상태를 만들면 라이트 규격 하나로 끝나지 않고 이 다크 주입 CSS에도 짝을 넣어야 게시본에서 의도대로 보인다.
  - **2026-08-22 눌림 상태 규격으로 교체**: 생성기 쪽 ★ 퀵필터 규격이 '항상 채움 + active=테두리'에서 **안 누름=색 테두리+색 별(빈 배경) / 누름=색 채움+볼드**로 바뀌면서([[page-market]]), 합성기의 다크 짝도 같은 4줄로 갈아끼웠다(종전 '흰 테두리 active' 규칙 폐기). 색은 하드코딩 대신 다크 팔레트 키를 쓴다 — 배경 `input-bg`, watchlist=`hl2-fg`(시안)·update=`hl3-fg`(에메랄드), 눌림 시 글자색은 각 `hl2-bg`/`hl3-bg`. 8/17 항목의 교훈("생성기에 새 버튼이 생기면 다크 CSS에도 짝을 넣어야 한다")이 **새 버튼뿐 아니라 기존 버튼의 상태 규격 변경에도 그대로 적용**된 사례다.
