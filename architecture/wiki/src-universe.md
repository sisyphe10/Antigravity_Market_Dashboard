...
- **암호화폐 3종 편입(2026-08-22)**: `PREFIX_MAP`에 `CRYPTO`(suffix 없음 · 통화 USD) prefix, `ALLOWED_SECTORS`에 섹터 라벨 `암호화폐`를 더해 `CRYPTO:BTC-USD`·`ETH-USD`·`SOL-USD`(비트코인/이더리움/솔라나)를 유니버스에 넣었다 — yfinance 심볼이 그대로 티커라 접미사 변환이 없는 첫 prefix.
  - **RSI(1M) 벤치마크 = BTC 자신**(사용자 확정): 대응 주가지수가 없어 `fetch_index_returns.py`의 `INDICES`에 `('BTC','BTC-USD')`를 지수로 등록하고, [[page-market]]·[[page-universe]]를 만드는 `create_dashboard.py`의 클라이언트측 `INDEX_BY_PREFIX`에 `CRYPTO → BTC`를 배선했다. BTC 행은 자기 자신 대비라 RSI(1M)이 0 부근에 붙고, ETH·SOL은 **BTC 상대강도**로 읽는 지표가 된다 — 지수 없는 자산군을 대표 자산으로 대체한 첫 사례라 해석 규칙이 다른 prefix와 다르다.
