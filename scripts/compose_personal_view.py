...
수행
  ...
  5) 검증        : sisyphe 페이지 존재·staticrypt 흔적 0·역방향 nav 주입 확인. 실패 시 exit1 → 호출측이 세대 폐기.
"""
...
if os.path.exists(wrap):
    fail("wrap.html 잔존")
# index.html(루트, TOP_NAV_MAIN 기준) 정방향 nav 는 필수 — 없으면 구조 이상
...
# --- 4) 역방향 nav 주입 + 검증 (멱등, fail-closed) ---
for name in SISYPHE_PAGES:
    ...
    if 'class="topnav-tab aoe-back"' not in open(p, encoding="utf-8").read():
        fail("sisyphe/%s: 역방향 nav 검증 실패" % name)

sys.stdout.write("[compose] OK: WRAP 제거 + 양방향 nav + Sisyphe %d페이지 합성 (%s)\n"
