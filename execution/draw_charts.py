...
def draw_charts():
    ...
    try:
        ...
        if '데이터 타입' in df.columns:
            ...
            df = df[~df['데이터 타입'].astype(str).str.startswith(('ECOS_', 'FRED_', 'ISM_', 'BOND_', 'ETF_'))]
        ...
        for name, group in grouped:
            ...
            if 'Trading Volume' in name:
                ax.bar(filtered_data['날짜'], filtered_data['가격'], color=LINE_COLOR, width=1.0, alpha=0.3, edgecolor='none')
            else:
            ...
            if len(filtered_data) >= 20:
                ...
                ax.plot(filtered_data['날짜'], ma20, color='#DC2626', linewidth=2, alpha=0.5)
            if len(filtered_data) >= 120:
                ma120 = filtered_data['가격'].rolling(window=120).mean()
                ax.plot(filtered_data['날짜'], ma120, color='#1B5E20', linewidth=2, alpha=0.5)
