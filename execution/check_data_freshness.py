...
소스별 최신일자 추출:
  ...
  - 루트 CSV(us_etf_history.csv 등): 0열 날짜 max
...
# ── 루트 CSV 시계열: 파일 → (모드, 임계) ────────────────────────────────────
#   us_etf_history.csv: 미국(+홍콩) ETF NAV·AUM — 맥미니 launchd 화~토 09:00 KST
#   수집, 일자=직전 미국 거래일. 정상 gap: 주말 3일·미국 월요일 휴장 최대 5일.
CSV_LAGGED = {'us_etf_history.csv': ('calendar', 6)}
...
LABELS = {
    ...
    'us_etf_history.csv': '미국 ETF NAV·AUM',
}
...
def csv_max_date(path: str):
    """단일 CSV(0열=날짜)의 최대 일자. 파일 없음/파싱 실패=None."""
    try:
        with open(path, encoding='utf-8-sig', newline='') as f:
            ...
            best = None
            for row in r:
                d = parse_d(row[0]) if row else None
...
def check(data_dir: str, today: date):
    ...
    for fn, (mode, thr) in CSV_LAGGED.items():
        add(LABELS.get(fn, fn), csv_max_date(os.path.join(data_dir, fn)), mode, thr)
    return alerts
