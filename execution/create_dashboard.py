    ...
    try:
        ...
        for sec_idx, (portfolio_name, data_key) in enumerate(render_list):
            ...
            <div class="portfolio-section" data-pf-sec="{sec_idx}" style="{sec_style}">
                <h3 class="portfolio-title">{portfolio_name} <span style="font-size:0.9rem;font-weight:400;color:#000;">({portfolio_updated})</span></h3>
...
def _load_aoe_core_js():
    ...
    sha 불일치 = 코어만 고치고 chart_core/build_core.py 를 안 돌린 것 — 생성을 거부해
    조용한 드리프트를 막는다 (DECISION 8/2). 검증 후 프로세스 내 캐시.
    """
    global _AOE_CORE_JS_CACHE
    if _AOE_CORE_JS_CACHE is None:
        import hashlib
        _root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _cpath = os.path.join(_root, 'chart_core', 'dist', 'aoe_chart.js')
        _mpath = os.path.join(_root, 'chart_core', 'dist', 'aoe_chart.manifest.json')
        with open(_cpath, encoding='utf-8') as _f:
            _core = _f.read()
        with open(_mpath, encoding='utf-8') as _f:
            _mani = json.load(_f)
        _sha = hashlib.sha256(_core.encode('utf-8')).hexdigest()
        if _sha != _mani.get('coreSha256'):
            raise RuntimeError(
                f"aoe_chart.js sha 불일치 ({_sha[:12]} != manifest {str(_mani.get('coreSha256'))[:12]}) "
                '— python3 chart_core/build_core.py 실행 후 재생성하세요')
        if '</script>' in _core.lower():
            raise RuntimeError('aoe_chart.js 에 </script> 포함 — 인라인 임베드 불가')
        _AOE_CORE_JS_CACHE = _core
    return _AOE_CORE_JS_CACHE


def _chart_download_helper_js():
    """공용 차트 코어 인라인 임베드 (Download/Copy 헬퍼 포함).

    중복 inject 가드는 코어 내부의 typeof 체크가 담당 — 여러 번 들어가도 1회만 등록.
    """
    return '<script>\n' + _load_aoe_core_js() + '\n        </script>'
...
def _series_country(group_label, s, group_country=None):
    ...
    if g == 'CAPEX':
        return 'Japan'
    if g in ('CRYPTO', 'CRYPTOCURRENCY', 'MEMORY'):
        return 'Global'
    if g == 'COMMODITIES':
...
INVFLOW_INVESTORS = ['개인', '외국인', '금융투자', '기관(금투제외)',
                     '연기금', '투신', '사모', '보험', '은행', '기타금융', '기타법인']
INVFLOW_BASE_COLORS = {
    '개인': '#C62828', '외국인': '#1565C0',
...
def _build_combined_chart_section():
    ...
    try:
        groups = [
            ...
            {'label': 'HOUSING', 'country': 'Korea', 'series': [
                # ECOS·KOSIS 부동산 (5년 임베드 창)
        ...
        (function() {
            ...
            function updateCmbGroupBadges() {
                ...
                document.querySelectorAll('.cmb-series-row .cmb-chart-item.active').forEach(function(el) {
                    seen[el.getAttribute('data-series')] = 1;
                });
                var n = Object.keys(seen).length;   // 별표 복제 행 중복 제거
            ...
            function applyMarkerColors() {
                document.querySelectorAll('.cmb-chart-item').forEach(function(el) {
                    ...
                    // 다크 스킨 선택 하이라이트 훅 (2026-08-03): 클릭 순번을 data-hl(1~3)로
                    // 행에 마킹 — 별표 복제 행과 원본이 항상 같은 색을 받는다.
            ...
            function buildCmbChart() {
                ...
                // ── 렌더 프레임 = 코어 cmbRenderCharts (P2b 2026-08-02) — 페이지는 뷰 조립만.
                //    차트 인스턴스는 이 클로저 소유(_cmbPeerAccessor)라 반환값을 되받아 보관한다.
                ...
                if (document.getElementById('cmbRocPanel')) {
            ...
            window.toggleCmbSeries = function(el, ev) {
                ...
                // 별표 복제 행(cmb-pin-clone)과 원본이 공존하므로 같은 시리즈의
                // 모든 복사본에 active 를 동기화한다 (판정 기준 = 시리즈 단위).
                var key = el.getAttribute('data-series');
                var same = [], anyOn = false;
                document.querySelectorAll('.cmb-chart-item').forEach(function(x) {
                    if (x.getAttribute('data-series') === key) {
                        same.push(x);
                        if (x.classList.contains('active')) anyOn = true;
                    }
                });
                if (multi) {
                    // 다중 선택 모드: 시리즈 토글
                    same.forEach(function(x) { x.classList.toggle('active', !anyOn); });
                } else {
                    // 단일 선택 모드: 다른 항목 모두 해제 + 현재 시리즈만 active
                    document.querySelectorAll('.cmb-chart-item.active').forEach(function(x) {
                        x.classList.remove('active');
                    });
                    same.forEach(function(x) { x.classList.add('active'); });
                    cmbClickOrder = [key];
                }
                window._cmbNavEl = el;   // 방향키 내비 시작점 = 마지막 클릭 위치
            ...
            document.addEventListener('keydown', function(e) {
                ...
                e.preventDefault();
                // 복제/원본 중복이 있어 '마지막 내비·클릭 위치'(_cmbNavEl)를 앵커로 우선 사용
                var activeIdx = -1;
                var nav = window._cmbNavEl;
                if (nav && nav.classList.contains('active')) activeIdx = items.indexOf(nav);
                if (activeIdx < 0) {
                ...
                document.querySelectorAll('.cmb-chart-item.active').forEach(function(el) { el.classList.remove('active'); });
                var nk = items[nextIdx].getAttribute('data-series');
                document.querySelectorAll('.cmb-chart-item').forEach(function(el) {
                    if (el.getAttribute('data-series') === nk) el.classList.add('active');
                });
                window._cmbNavEl = items[nextIdx];
                cmbClickOrder = [nk];
            ...
            <div style="display:flex;gap:16px;align-items:flex-start;max-width:1800px;margin:0 auto;justify-content:center;">
                ...
                <div style="width:1000px;">
                    <div style="display:flex;gap:8px;align-items:center;margin-bottom:12px;font-size:13px;">
                        ...
                        <button onclick="clearCmbSelections()" style="font-family:inherit;font-size:13px;font-weight:600;padding:4px 14px;background:#f3f4f6;color:#444;border:1px solid #d1d5db;border-radius:6px;cursor:pointer;">전체 해제</button>
                        <button onclick="copyChartImage('cmbDynamicChart','cmbChartLegend',['cmbDispChart','cmbRocChart'],this)" style="margin-left:auto;font-family:inherit;font-size:13px;font-weight:600;padding:6px 14px;background:#0891b2;color:#fff;border:none;border-radius:8px;cursor:pointer;">Copy</button>
                        <button onclick="downloadChartImage('cmbDynamicChart','AoE_Data','cmbChartLegend',['cmbDispChart','cmbRocChart'])" style="font-family:inherit;font-size:13px;font-weight:600;padding:6px 14px;background:#dc2626;color:#fff;border:none;border-radius:8px;cursor:pointer;">Download</button>
                    </div>
                    <style>
...
# (P7 2026-08-02: 미호출 데드 코드 _build_target_transform_chart_section 삭제 — BASELINE 실사 기록 참조)
    ...
    _html = """
        ...
        function renderOrderMatrix(){
          ...
          function wcell(pf,code,isNew,ci){ var cell=_mtxCell(pf,code); var val=cell.held?cell.newWeight:''; var gen=ORDER_PORTFOLIOS.filter(function(p){return p.display===pf;})[0].general;
            return '<td class="wcol '+(gen?'grp-general':'grp-target')+'"><input type="text" inputmode="decimal" class="cell mtx-w" data-code="'+escapeHtml(code)+'" data-pf="'+escapeHtml(pf)+'" data-col="'+ci+'"'+(isNew?' data-newrow="1"':'')+' value="'+(val===''?'':val)+'" style="color:'+cell.color+';"></td>'; }
          var rows='', rank=0;
          ...
          ORDER_PORTFOLIOS.forEach(function(p){ var _m=(p.general&&_fg)?' data-firstgen="1"':''; if(p.general)_fg=false; var _mt=(!p.general&&_ft)?' data-firsttarget="1"':''; if(!p.general)_ft=false; head+='<th class="wcol '+(p.general?'grp-general':'grp-target')+'"'+_m+_mt+' title="'+escapeHtml(p.display)+'" style="min-width:64px;">'+escapeHtml(p.label)+'</th>'; });
...
# (P7 2026-08-02: 미호출 데드 코드 _build_landing_kofia_section 삭제 — BASELINE 실사 기록 참조)
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        #tab0 .uv-star-th { color: #67e0f4 !important; }
        #tab0 .uv-star.on, #tab0 .uv-star-th.on { color: #67e0f4 !important; text-shadow: 0 0 6px rgba(103,224,244,.55); }
        #tab0 .uv-star:hover, #tab0 .uv-star-th:hover { color: #a5ecfa !important; }
...
<div class="content">
    ...
    <div class="filters" id="uvFilters" style="position:relative;">
