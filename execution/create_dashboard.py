...
<body>
    ...
    (function() {{
        ...
        function renderSpark(spark) {{
            ...
            // 축 라벨 여백: 좌=Y값, 하=X날짜
            var w = 560, h = 300, mL = 56, mR = 12, mT = 10, mB = 26;
            ...
            for (var yi = 0; yi < 3; yi++) {{
                ...
                svg += '<text x="' + (mL - 6) + '" y="' + (yy + 4).toFixed(1) + '" text-anchor="end" font-size="12" fill="#000">' + fmtVal(yLv[yi]) + '</text>';
            }}
        ...
        // 타이틀: "편의점 매출증감률 (+4.8%p)" — 이름 + 창 전체 변동폭 (pct 계열=%p, 레벨=단위)
        function renderTitle(slot) {{
            ...
            var chg = s[s.length - 1] - s[0];
            var chgTxt = (chg >= 0 ? '+' : '') + fmtVal(chg) + (isPct ? '%p' : (unit ? ' ' + unit : ''));
            nameEl.textContent = name + ' (' + chgTxt + ')';
        }}
