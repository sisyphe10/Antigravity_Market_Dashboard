            ...
            function buildChart() {
                ...
                wrapChart = new Chart(document.getElementById('wrapDynamicChart'), {
                    ...
                    options: {
                        ...
                        scales: {
                            x: { type: 'category', ticks: { callback: function(val, idx) { var d = this.getLabelForValue(val); if (!d) return ''; var parts = d.split('-'); var m = parseInt(parts[1]); var day = parseInt(parts[2]); if ((m === 1 || m === 7) && day <= 5) return parts[0].slice(2) + '/' + parts[1]; return ''; }, autoSkip: false, maxRotation: 0, font: { size: 11 }, color: '#888' }, grid: { display: false } },
