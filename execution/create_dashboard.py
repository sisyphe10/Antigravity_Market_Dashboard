    ...
    _html = """
        ...
        function renderEmailPanel() {
            ...
            TARGET_TABS.forEach(function(tt) {
                ...
                cells.push(buildEmailBox(pair.broker + ' 이메일', tEmail, '#f9fafb', '#e5e7eb', '#444', '#374151'));
            ...
            STANDALONE_GENERAL.forEach(function(sg) {
                ...
                cells.push(buildEmailBox(sg.broker + ' 이메일', sgEmail, '#f9fafb', '#e5e7eb', '#444', '#374151'));
        ...
        // ═══════════════════════════════════════════════════════════════
        // 통합 주문 매트릭스 (2026-07-13): per-portfolio orderStocks/orderState 위의 VIEW.
        //   행=전 포트 종목코드 합집합 + 신규행(matrixNewRows), 열=ORDER_PORTFOLIOS(동적).
        //   셀=변경후 입력 1개(변경전 미표시; 변경 시 숫자 색만: 편입/확대 #FF0000, 축소/편출 #0070C0).
        //   저장/복원/finalize/email/excel = 무수정(같은 per-portfolio 모델을 읽음).
        // ═══════════════════════════════════════════════════════════════
        var matrixNewRows = [];   // [{tempId, code, name, sector, reason}] 아직 어느 포트에도 없는 신규행
        var _mtxSeq = 0;
        var ORDER_COL_LABEL = {};
        ORDER_PORTFOLIOS.forEach(function(p){ ORDER_COL_LABEL[p.display] = (p.display.indexOf(' / ') >= 0) ? '일반형 결합' : p.display; });

        function _mtxFindIdx(pf, code) {
            var arr = orderStocks[pf] || [], c = String(code || '').trim();
            for (var i = 0; i < arr.length; i++) { if (String(arr[i].code || '').trim() === c) return i; }
            return -1;
        }
        function _mtxNewRowCodes() {
            var set = {};
            matrixNewRows.forEach(function(r){ var c = String(r.code || '').trim(); if (c) set[c] = true; });
            return set;
        }
        // 합집합 행: 코드별 1행, 첫 등장 포트의 name/sector 채택, 등장 순서. 신규행 소유 코드는 제외(신규행 섹션에서 렌더).
        function _mtxUnion() {
            var owned = _mtxNewRowCodes(), order = [], meta = {};
            ORDER_PORTFOLIOS.forEach(function(p){
                (orderStocks[p.display] || []).forEach(function(s){
                    var c = String(s.code || '').trim();
                    if (!c || owned[c]) return;
                    if (!meta[c]) { meta[c] = { code:c, name:s.name || '', sector:s.sector || '' }; order.push(c); }
                    else { if (!meta[c].name && s.name) meta[c].name = s.name; if (!meta[c].sector && s.sector) meta[c].sector = s.sector; }
                });
            });
            return { order:order, meta:meta };
        }
        function _mtxReason(code) {
            var c = String(code || '').trim();
            for (var pi = 0; pi < ORDER_PORTFOLIOS.length; pi++) {
                var pf = ORDER_PORTFOLIOS[pi].display, arr = orderStocks[pf] || [], st = orderState[pf] || [];
                for (var i = 0; i < arr.length; i++) { if (String(arr[i].code || '').trim() === c) { var r = (st[i] && st[i].reason || '').toString(); if (r.trim()) return r; } }
            }
            var nr = matrixNewRows.filter(function(r){ return String(r.code || '').trim() === c; })[0];
            return nr ? (nr.reason || '') : '';
        }
        // 코드 사유를 전 포트 orderState + 신규행에 일괄 기록 (코드레벨 동기화)
        function _mtxSetReason(code, reason) {
            var c = String(code || '').trim();
            ORDER_PORTFOLIOS.forEach(function(p){
                var arr = orderStocks[p.display] || [], st = orderState[p.display] || [];
                for (var i = 0; i < arr.length; i++) { if (String(arr[i].code || '').trim() === c && st[i]) st[i].reason = reason; }
            });
            matrixNewRows.forEach(function(r){ if (String(r.code || '').trim() === c) r.reason = reason; });
        }
        function _mtxColSum(pf) {
            var st = orderState[pf] || [], sum = 0;
            st.forEach(function(x){ sum += parseFloat(x.newWeight) || 0; });
            return sum;
        }
        // 셀 색/주문구분 (pf,code 자기 변경전 기준). held=false면 미보유(빈칸).
        function _mtxCell(pf, code) {
            var idx = _mtxFindIdx(pf, code);
            if (idx < 0) return { held:false, color:'#000', ot:'', newWeight:null };
            var prev = parseFloat(orderStocks[pf][idx].weight) || 0;
            var neww = parseFloat(orderState[pf][idx].newWeight) || 0;
            var ot = calcOrderType(prev, neww);
            var color = (ot === '신규 편입' || ot === '비중 확대') ? '#FF0000' : ((ot === '비중 축소' || ot === '전량 편출') ? '#0070C0' : '#000');
            return { held:true, color:color, ot:ot, newWeight:neww };
        }
        // 코드에 변경(유지 아님)이 하나라도 있는지 (사유 필요 판정)
        function _mtxCodeChanged(code) {
            for (var pi = 0; pi < ORDER_PORTFOLIOS.length; pi++) {
                var c = _mtxCell(ORDER_PORTFOLIOS[pi].display, code);
                if (c.held && c.ot && c.ot !== '유지') return true;
            }
            return false;
        }
        // 전역 저장 배지: 어느 포트든 dirty면 미저장, 아니면 전 포트 중 최신 저장상태.
        function buildOrderSaveBadgeGlobal() {
            var anyDirty = ORDER_PORTFOLIOS.some(function(p){ return orderIsDirty(p.display); });
            var kind, disp = null;
            if (anyDirty) { kind = 'dirty'; }
            else {
                var best = null;
                ORDER_PORTFOLIOS.forEach(function(p){
                    var s = computeOrderSaveState(p.display);
                    if (s && s.kind !== 'dirty' && (best === null || (s.ms || 0) > (best.ms || 0))) best = s;
                });
                if (!best) return '';
                kind = best.kind; disp = best.disp;
            }
            var bg, bd, txt;
        ...
        // 셀 편집 → per-portfolio 모델 반영 (코드 기반). raw='' 이면: 매트릭스 생성분은 제거, 보유분은 0(편출).
        function _mtxApplyCell(pf, code, raw, rowMeta) {
            var c = String(code || '').trim();
            if (!c) return;
            var idx = _mtxFindIdx(pf, c);
            var s = raw == null ? '' : String(raw).trim();
            if (s === '') {
                if (idx >= 0) {
                    var stk = orderStocks[pf][idx];
                    if (stk._mtxAdded && (parseFloat(stk.weight) || 0) === 0) {
                        orderStocks[pf].splice(idx, 1); orderState[pf].splice(idx, 1);   // 매트릭스 생성분 → 미보유 복귀
                    } else {
                        orderState[pf][idx].newWeight = 0;                                // portfolio_data 보유분 → 전량편출
                    }
                }
                return;
            }
            ...
            if (idx >= 0) {
                orderState[pf][idx].newWeight = val;
            } else {
                var m = rowMeta || _mtxUnion().meta[c] || { code:c, name:'', sector:'' };
                orderStocks[pf].push({ code:c, name:m.name || '', sector:m.sector || '', weight:0, isNew:true, _mtxAdded:true });
                orderState[pf].push({ newWeight:val, reason:_mtxReason(c) });
            }
        }
        ...
        function renderOrderMatrix() {
            var union = _mtxUnion();
            var pfs = ORDER_PORTFOLIOS.map(function(p){ return p.display; });
            var colCount = pfs.length;

            function weightCellHtml(pf, code, isNewRow) {
                var cell = _mtxCell(pf, code);
                var val = cell.held ? cell.newWeight : '';
                return '<td style="text-align:center;padding:6px 4px;">'
                    + '<input type="text" inputmode="decimal" class="mtx-w" data-code="' + escapeHtml(code) + '" data-pf="' + escapeHtml(pf) + '"'
                    + (isNewRow ? ' data-newrow="1"' : '')
                    + ' value="' + (val === '' ? '' : val) + '"'
                    + ' style="width:58px;box-sizing:border-box;text-align:center;padding:4px 4px;border:1px solid #d1d5db;border-radius:4px;font-family:inherit;font-size:15px;font-weight:600;color:' + cell.color + ';"></td>';
            }

            var rows = '';
            var rank = 0;
            // 합집합 행
            union.order.forEach(function(code){
                rank++;
                var m = union.meta[code];
                var reason = _mtxReason(code);
                var reasonNeeded = _mtxCodeChanged(code) && !reason.trim();
                var cells = '';
                pfs.forEach(function(pf){ cells += weightCellHtml(pf, code, false); });
                rows += '<tr>'
                    + '<td style="text-align:center;padding:6px 4px;color:#666;">' + rank + '</td>'
                    + '<td style="text-align:center;padding:6px 4px;">' + escapeHtml(m.sector || '') + '</td>'
                    + '<td style="text-align:center;padding:6px 4px;">' + escapeHtml(m.code) + '</td>'
                    + '<td style="text-align:center;font-weight:600;padding:6px 4px;">' + escapeHtml(m.name || '') + '</td>'
                    + cells
                    + '<td style="padding:6px 4px;"><input type="text" class="mtx-reason" data-code="' + escapeHtml(code) + '" value="' + escapeHtml(reason).replace(/"/g, '&quot;') + '" style="width:100%;box-sizing:border-box;padding:4px 6px;border:1px solid ' + (reasonNeeded ? '#fca5a5' : '#d1d5db') + ';background:' + (reasonNeeded ? '#fee2e2' : '#fff') + ';border-radius:4px;font-family:inherit;font-size:14px;text-align:center;color:#000;"></td>'
                    + '<td style="padding:6px 4px;"></td>'
                    + '</tr>';
            });
            // 신규행 (matrixNewRows)
            matrixNewRows.forEach(function(r){
                rank++;
                var code = String(r.code || '').trim();
                var reasonNeeded = code && _mtxCodeChanged(code) && !(r.reason || '').trim();
                var cells = '';
                pfs.forEach(function(pf){ cells += weightCellHtml(pf, code, true); });
                rows += '<tr data-tempid="' + r.tempId + '">'
                    + '<td style="text-align:center;padding:6px 4px;color:#666;">' + rank + '</td>'
                    + '<td style="text-align:center;padding:6px 4px;"><input type="text" class="mtx-meta" data-tempid="' + r.tempId + '" data-field="sector" value="' + escapeHtml(r.sector || '').replace(/"/g, '&quot;') + '" placeholder="업종" style="width:96px;text-align:center;padding:4px 6px;border:1px solid #d1d5db;border-radius:4px;font-family:inherit;font-size:14px;color:#000;"></td>'
                    + '<td style="text-align:center;padding:6px 4px;"><input type="text" class="mtx-meta" data-tempid="' + r.tempId + '" data-field="code" value="' + escapeHtml(r.code || '') + '" placeholder="000000" maxlength="6" style="width:66px;text-align:center;padding:4px 6px;border:1px solid #d1d5db;border-radius:4px;font-family:inherit;font-size:14px;color:#000;"></td>'
                    + '<td style="text-align:center;padding:6px 4px;"><input type="text" class="mtx-meta" data-tempid="' + r.tempId + '" data-field="name" value="' + escapeHtml(r.name || '').replace(/"/g, '&quot;') + '" placeholder="종목명" style="width:116px;text-align:center;padding:4px 6px;border:1px solid #d1d5db;border-radius:4px;font-family:inherit;font-size:14px;font-weight:600;color:#000;"></td>'
                    + cells
                    + '<td style="padding:6px 4px;"><input type="text" class="mtx-reason" data-code="' + escapeHtml(code) + '" data-tempid="' + r.tempId + '" value="' + escapeHtml(r.reason || '').replace(/"/g, '&quot;') + '" style="width:100%;box-sizing:border-box;padding:4px 6px;border:1px solid ' + (reasonNeeded ? '#fca5a5' : '#d1d5db') + ';background:' + (reasonNeeded ? '#fee2e2' : '#fff') + ';border-radius:4px;font-family:inherit;font-size:14px;text-align:center;color:#000;"></td>'
                    + '<td style="text-align:center;padding:6px 4px;"><button class="mtx-del-btn" data-tempid="' + r.tempId + '" title="행 삭제" style="background:none;border:none;cursor:pointer;color:#dc2626;font-size:20px;line-height:1;padding:0 6px;font-weight:bold;">×</button></td>'
                    + '</tr>';
            });

            // 합계 행 (포트 열별 newWeight 합 + 100% 색)
            var totalCells = '';
            pfs.forEach(function(pf, ci){
                var sum = _mtxColSum(pf);
                var col = (Math.abs(sum - 100) < 0.5) ? '#16a34a' : (sum > 100 ? '#dc2626' : '#222');
                totalCells += '<td id="mtx-total-' + ci + '" style="padding:8px 4px;text-align:center;color:' + col + ';">' + sum.toFixed(0) + '%</td>';
            });
            ...
            var headCols = '';
            pfs.forEach(function(pf){ headCols += '<th title="' + escapeHtml(pf) + '" style="padding:8px 4px;text-align:center;min-width:66px;">' + escapeHtml(ORDER_COL_LABEL[pf]) + '</th>'; });
            ...
            var html = '<div style="background:#fff;border-radius:12px;padding:24px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">'
                ...
                + '<h3 style="margin:0;font-size:18px;">주문 매트릭스</h3>'
                ...
                + '<th style="padding:8px 4px;text-align:center;">업종</th>'
                + '<th style="padding:8px 4px;text-align:center;width:66px;">코드</th>'
                + '<th style="padding:8px 4px;text-align:center;">종목명</th>'
                + headCols
                + '<th style="padding:8px 4px;text-align:center;">추천사유</th>'
                + '<th style="padding:8px 4px;text-align:center;width:36px;"></th>'
            ...
            var _prevFocus = _snapshotMtxFocus();
            document.getElementById('orderContent').innerHTML = html;
            _bindMtxHandlers();
            _restoreMtxFocus(_prevFocus);
        }

        function _mtxUpdateTotals() {
            ORDER_PORTFOLIOS.forEach(function(p, ci){
                var cell = document.getElementById('mtx-total-' + ci);
                if (!cell) return;
                var sum = _mtxColSum(p.display);
                var col = (Math.abs(sum - 100) < 0.5) ? '#16a34a' : (sum > 100 ? '#dc2626' : '#222');
                cell.style.color = col; cell.textContent = sum.toFixed(0) + '%';
            });
        }

        function _bindMtxHandlers() {
            // 비중 셀: 입력 → 모델 반영 → 전체 재렌더(색·합계 갱신) + 포커스 복원 (기존 검증 패턴)
            document.querySelectorAll('#orderContent input.mtx-w').forEach(function(el){
                el.addEventListener('input', function(e){
                    var code = e.target.dataset.code, pf = e.target.dataset.pf;
                    var rowMeta = null;
                    if (e.target.dataset.newrow) {
                        var tr = e.target.closest('tr'); var tid = tr && tr.dataset.tempid;
                        var nr = matrixNewRows.filter(function(r){ return String(r.tempId) === String(tid); })[0];
                        if (nr) { code = String(nr.code || '').trim(); rowMeta = nr; }
                    }
                    if (!code) { return; }   // 신규행 코드 미입력 → 무시(코드 먼저)
                    _mtxApplyCell(pf, code, e.target.value, rowMeta);
                    renderOrderMatrix();
                    refreshOrderSaveBadge();
                });
            });
            // 추천사유: 재렌더 없이 모델 전파 + 배지 (타이핑 중 포커스 보존)
            document.querySelectorAll('#orderContent input.mtx-reason').forEach(function(el){
                el.addEventListener('input', function(e){
                    var code = e.target.dataset.code, tid = e.target.dataset.tempid;
                    if ((!code || !code.trim()) && tid) {
                        var nr = matrixNewRows.filter(function(r){ return String(r.tempId) === String(tid); })[0];
                        if (nr) nr.reason = e.target.value;
                    } else if (code) {
                        _mtxSetReason(code, e.target.value);
                    }
                    refreshOrderSaveBadge();
                });
                el.addEventListener('paste', handleMtxReasonPaste);
            });
            // 신규행 메타(업종/코드/종목명)
            document.querySelectorAll('#orderContent input.mtx-meta').forEach(function(el){
                el.addEventListener('input', function(e){
                    var tid = e.target.dataset.tempid, field = e.target.dataset.field;
                    var nr = matrixNewRows.filter(function(r){ return String(r.tempId) === String(tid); })[0];
                    if (nr) nr[field] = e.target.value;
                });
            });
            document.querySelectorAll('#orderContent input.mtx-meta[data-field="name"]').forEach(function(el){
                el.addEventListener('blur', function(e){ autoFillMtxRow(e.target.dataset.tempid, 'name'); });
            });
            document.querySelectorAll('#orderContent input.mtx-meta[data-field="code"]').forEach(function(el){
                el.addEventListener('blur', function(e){ autoFillMtxRow(e.target.dataset.tempid, 'code'); });
            });
            // 신규행 삭제
            document.querySelectorAll('#orderContent .mtx-del-btn').forEach(function(btn){
                btn.addEventListener('click', function(e){
                    var tid = e.currentTarget.dataset.tempid;
                    var nr = matrixNewRows.filter(function(r){ return String(r.tempId) === String(tid); })[0];
                    if (nr && String(nr.code || '').trim()) {
                        var c = String(nr.code).trim();
                        ORDER_PORTFOLIOS.forEach(function(p){
                            var i = _mtxFindIdx(p.display, c);
                            if (i >= 0 && orderStocks[p.display][i]._mtxAdded) { orderStocks[p.display].splice(i, 1); orderState[p.display].splice(i, 1); }
                        });
                    }
                    matrixNewRows = matrixNewRows.filter(function(r){ return String(r.tempId) !== String(tid); });
                    renderOrderMatrix();
                });
            });
            // 전역 버튼
            ...
            if (finalizeBtn) finalizeBtn.addEventListener('click', function(){ finalizeOrder(null); });
            ...
            if (addBtn) addBtn.addEventListener('click', function(){ addOrderStock(null); });
        }

        // ── 포커스 스냅샷/복원 (코드 기반) ──
        function _snapshotMtxFocus() {
            var ae = document.activeElement;
            if (!ae || ae.tagName !== 'INPUT' || !ae.dataset || !ae.closest || !ae.closest('#orderContent')) return null;
            var snap = { code: ae.dataset.code || null, pf: ae.dataset.pf || null, tempid: ae.dataset.tempid || null, field: ae.dataset.field || null, cls: ae.className, start: null, end: null };
            try { snap.start = ae.selectionStart; snap.end = ae.selectionEnd; } catch (e) {}
            return snap;
        }
        function _restoreMtxFocus(snap) {
            if (!snap) return;
            var sel = '#orderContent input';
            var el = null;
            if (snap.cls && snap.cls.indexOf('mtx-w') >= 0 && snap.code != null && snap.pf != null) {
                el = document.querySelector('#orderContent input.mtx-w[data-code="' + snap.code + '"][data-pf="' + snap.pf + '"]');
            } else if (snap.cls && snap.cls.indexOf('mtx-reason') >= 0) {
                if (snap.tempid) el = document.querySelector('#orderContent input.mtx-reason[data-tempid="' + snap.tempid + '"]');
                if (!el && snap.code != null) el = document.querySelector('#orderContent input.mtx-reason[data-code="' + snap.code + '"]');
            } else if (snap.cls && snap.cls.indexOf('mtx-meta') >= 0 && snap.tempid && snap.field) {
                el = document.querySelector('#orderContent input.mtx-meta[data-tempid="' + snap.tempid + '"][data-field="' + snap.field + '"]');
            }
            if (!el) return;
            el.focus();
            if (snap.start != null) { try { el.setSelectionRange(snap.start, snap.end); } catch (e) {} }
        }
        // 추천사유 멀티라인 붙여넣기 → 화면상 다음 추천사유 칸들로 순서 분배
        function handleMtxReasonPaste(e) {
            ...
            e.preventDefault();
            var all = Array.prototype.slice.call(document.querySelectorAll('#orderContent input.mtx-reason'));
            var startPos = all.indexOf(e.target);
            if (startPos < 0) return;
            lines.forEach(function(line, k){
                var t = all[startPos + k];
                if (!t) return;
                t.value = line;
        ...
        // + 종목 추가 → 신규행(transient) 1개. 코드/종목명/업종 입력 후 원하는 포트 셀에 비중 입력.
        function addOrderStock(_ignored) {
            matrixNewRows.push({ tempId: 'mtx' + (++_mtxSeq), code: '', name: '', sector: '', reason: '' });
            renderOrderMatrix();
        }

        // 신규행 종목명/코드 입력 후 blur → stock_master.json lookup으로 나머지 + 업종 자동
        async function autoFillMtxRow(tempId, source) {
            var nr = matrixNewRows.filter(function(r){ return String(r.tempId) === String(tempId); })[0];
            if (!nr) return;
            try {
                await loadWicsAll();
                if (source === 'name') {
                    var name = String(nr.name || '').trim();
                    if (!name) return;
                    var lower = name.toLowerCase();
                    var hit = wicsByName[name]
                        || wicsAllCache.find(function(x){ return x.name && x.name.toLowerCase() === lower; })
                        || wicsAllCache.find(function(x){ return x.name && x.name.toLowerCase().indexOf(lower) >= 0; });
                    if (!hit) return;
                    if (!String(nr.code || '').trim()) nr.code = hit.code;
                    nr.name = hit.name;
                    if (!String(nr.sector || '').trim()) { var sec = lookupSectorByCode(nr.code) || hit.sector || ''; if (sec) nr.sector = sec; }
                } else {
                    var code = String(nr.code || '').trim();
                    if (!code) return;
                    if (/^\\d{1,6}$/.test(code) && code.length < 6) { code = code.padStart(6, '0'); nr.code = code; }
                    var hit2 = wicsByCode[code];
                    if (!hit2) return;
                    if (!String(nr.name || '').trim()) nr.name = hit2.name;
                    if (!String(nr.sector || '').trim()) { var sec2 = lookupSectorByCode(nr.code) || hit2.sector || ''; if (sec2) nr.sector = sec2; }
                }
                renderOrderMatrix();
            } catch (e) { console.warn('autoFillMtxRow 실패:', e); }
        }
        ...
        function additionalOrder() {
            ...
            matrixNewRows.forEach(function(r){ r.reason = ''; });
            if (orderActiveTab && orderActiveTab !== 'Email') renderOrderMatrix();
        }
