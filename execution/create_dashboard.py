...
def _build_combined_chart_section():
    ...
    try:
        ...
        """
        # ★legacy 롤백 경로 (P2b, DECISION 8/2): AOE_CHART_LEGACY=1 이면 코어 이관 직전
        #   동결 렌더러(legacy_chart_renderers.py)로 현재 데이터를 재생성한다. P7에서 제거.
        if os.environ.get('AOE_CHART_LEGACY') == '1':
            import importlib.util as _ilu
            _lp = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'legacy_chart_renderers.py')
            _spec = _ilu.spec_from_file_location('legacy_chart_renderers', _lp)
            _legacy = _ilu.module_from_spec(_spec)
            _spec.loader.exec_module(_legacy)
            js_code = _legacy.LEGACY_CMB_JS
