...
function refresh() {{
    ...
    absList.forEach(function(r, i) {{
        ...
        var chgLabel = isSingle ? ((avgChg > 0 ? '+' : '') + avgChg.toFixed(1) + '%') : ((avgChg > 0 ? '+' : '') + avgChg.toFixed(1) + '%');
    ...
    turnList.forEach(function(r, i) {{
        var avgChg = r.cnt > 0 ? (r.chgSum / r.cnt) : 0;
        var cls = avgChg > 0 ? 'pos' : (avgChg < 0 ? 'neg' : '');
        var chgLabel = (avgChg > 0 ? '+' : '') + avgChg.toFixed(1) + '%';
        var avgTurnover = r.cnt > 0 ? (r.turnover / r.cnt) : 0;
        h2 += '<tr><td class="c">' + (i+1) + '</td><td class="c">' + r.name + '</td><td class="c">' + r.market + '</td><td class="c">' + fmtVal(r.trdval) + '</td><td class="c">' + avgTurnover.toFixed(1) + '%</td><td class="c">' + fmtVal(r.mktcap) + '</td><td class="c ' + cls + '">' + chgLabel + '</td></tr>';
    }});
