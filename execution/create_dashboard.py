        ...
        async function downloadOrderExcel(pfName, templateIdx) {
            ...
            try {
                ...
                combined.forEach(function(s, i) {
                    ...
                    var ot = s.orderType;
                    // 글씨는 항상 검정. 배경: 편입/확대=옅은 빨강, 축소/편출=옅은 파랑, 유지=흰색(템플릿 기본).
                    var prevFont = hCell.font || {};
                    hCell.font = Object.assign({}, prevFont, { color: { argb: 'FF000000' } });
                    var bgColor;
                    if (ot === '신규 편입' || ot === '비중 확대') bgColor = 'FFFFC7CE';
                    else if (ot === '비중 축소' || ot === '전량 편출') bgColor = 'FFBDD7EE';
...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .lh-shuffle {{ flex: 0 0 auto; background: none; border: none; cursor: pointer; font-size: 2.1rem; opacity: 0.9; padding: 8px 12px; transition: opacity 0.15s, transform 0.2s; line-height: 1; color: inherit; }}
