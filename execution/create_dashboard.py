...
<body>
    ...
    <div class="wrap-tabs">
        <button class="wrap-tab active" onclick="switchWrapTab('wrap')">WRAP</button>
        <button class="wrap-tab" onclick="switchWrapTab('universe')">Universe</button>
    </div>

    <div id="wrapTabContent" class="wrap-tab-content">
    {wrap_html}
    </div>

    <div id="universeTabContent" class="wrap-tab-content" style="display:none;">
        <div class="category-section">
            <h2 class="category-title">Universe</h2>
            <div style="margin-bottom:12px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                <select id="uniFilterCurrency" onchange="filterUniverse()" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;">
                    <option value="">통화 전체</option>
                </select>
                <select id="uniFilterSector1" onchange="filterUniverse()" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;">
                    <option value="">섹터1 전체</option>
                </select>
                <select id="uniFilterSector2" onchange="filterUniverse()" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;">
                    <option value="">섹터2 전체</option>
                </select>
            </div>
            <div style="overflow-x:auto;">
                <table class="portfolio-table" id="universeTable">
                    <thead><tr>
                        <th onclick="sortUniverse(0)">#</th>
                        <th onclick="sortUniverse(1)">통화</th>
                        <th onclick="sortUniverse(2)">섹터 1</th>
                        <th onclick="sortUniverse(3)">섹터 2</th>
                        <th onclick="sortUniverse(4)">티커</th>
                        <th onclick="sortUniverse(5)">기업명</th>
                        <th onclick="sortUniverse(6)">시가총액</th>
                        <th onclick="sortUniverse(7)">가격</th>
                        <th onclick="sortUniverse(8)">YTD</th>
                        <th onclick="sortUniverse(9)">1D</th>
                        <th onclick="sortUniverse(10)">1W</th>
                        <th onclick="sortUniverse(11)">1M</th>
                        <th onclick="sortUniverse(12)">3M</th>
                        <th onclick="sortUniverse(13)">6M</th>
                        <th onclick="sortUniverse(14)">1Y</th>
                    </tr></thead>
                    <tbody id="universeBody">
                        <tr><td colspan="15" style="text-align:center;padding:40px;color:#888;">로딩 중...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    // Tab switching
    function switchWrapTab(tab) {{
        document.querySelectorAll('.wrap-tab').forEach(function(b) {{ b.classList.remove('active'); }});
        event.target.classList.add('active');
        document.getElementById('wrapTabContent').style.display = tab === 'wrap' ? '' : 'none';
        document.getElementById('universeTabContent').style.display = tab === 'universe' ? '' : 'none';
        if (tab === 'universe' && !window._uniLoaded) loadUniverse();
    }}

    // Universe
    var _uniData = [];
    var _uniSortCol = -1;
    var _uniSortAsc = true;

    function loadUniverse() {{
        window._uniLoaded = true;
        var url = 'https://sheets.googleapis.com/v4/spreadsheets/1KR9RJN53G-yJtnowQbg5bcAiIBfrkIeNqN_PO2UOCTM/values/%EC%8B%9C%ED%8A%B81?key=AIzaSyCHPiRby5FVAIKDwneZHy1KGl3SfycjZEw';
        fetch(url).then(function(r) {{ return r.json(); }}).then(function(data) {{
            var rows = (data.values || []).slice(1);
            _uniData = rows.map(function(r) {{ return r.slice(0, 15); }});
            populateFilters();
            renderUniverse();
        }});
    }}
    ...
    function populateFilters() {{
        var currencies = {{}}, s1 = {{}}, s2 = {{}};
        _uniData.forEach(function(r) {{
            if (r[1]) currencies[r[1]] = 1;
            if (r[2]) s1[r[2]] = 1;
            if (r[3]) s2[r[3]] = 1;
        }});
        var sel1 = document.getElementById('uniFilterCurrency');
        var sel2 = document.getElementById('uniFilterSector1');
        var sel3 = document.getElementById('uniFilterSector2');
        sel1.innerHTML = '<option value="">통화 전체</option>' + Object.keys(currencies).sort().map(function(v) {{ return '<option>' + v + '</option>'; }}).join('');
        sel2.innerHTML = '<option value="">섹터1 전체</option>' + Object.keys(s1).sort().map(function(v) {{ return '<option>' + v + '</option>'; }}).join('');
        sel3.innerHTML = '<option value="">섹터2 전체</option>' + Object.keys(s2).sort().map(function(v) {{ return '<option>' + v + '</option>'; }}).join('');
    }}

    function filterUniverse() {{ renderUniverse(); }}

    function sortUniverse(col) {{
        if (_uniSortCol === col) {{ _uniSortAsc = !_uniSortAsc; }}
        else {{ _uniSortCol = col; _uniSortAsc = true; }}
        // Update header indicators
        document.querySelectorAll('#universeTable thead th').forEach(function(th, i) {{
            th.style.cursor = 'pointer';
            var base = ['#','통화','섹터 1','섹터 2','티커','기업명','시가총액','가격','YTD','1D','1W','1M','3M','6M','1Y'][i];
            th.textContent = i === col ? base + (_uniSortAsc ? ' ▲' : ' ▼') : base;
        }});
        renderUniverse();
    }}
    ...
    function renderUniverse() {{
        var fc = document.getElementById('uniFilterCurrency').value;
        var fs1 = document.getElementById('uniFilterSector1').value;
        var fs2 = document.getElementById('uniFilterSector2').value;
        var filtered = _uniData.filter(function(r) {{
        ...
        if (_uniSortCol >= 0) {{
            ...
            var isNum = numCols.indexOf(col) >= 0;
            filtered.sort(function(a, b) {{
                var va = isNum ? parseNum(a[col]) : (a[col] || '');
                var vb = isNum ? parseNum(b[col]) : (b[col] || '');
                if (va < vb) return _uniSortAsc ? -1 : 1;
                if (va > vb) return _uniSortAsc ? 1 : -1;
                return 0;
            }});
        }}
        ...
        filtered.forEach(function(r) {{
            ...
            for (var i = 0; i < 15; i++) {{
                var v = r[i] || '';
                var cls = '';
                if (pctCols.indexOf(i) >= 0 && v) {{
                    var n = parseFloat(v.replace(/%/g, ''));
                    if (!isNaN(n)) cls = n > 0 ? ' class="positive"' : n < 0 ? ' class="negative"' : '';
                }}
                html += '<td' + cls + '>' + v + '</td>';
            }}
            html += '</tr>';
        }});
        if (!html) html = '<tr><td colspan="15" style="text-align:center;padding:40px;color:#888;">데이터 없음</td></tr>';
        document.getElementById('universeBody').innerHTML = html;
    }}
    </script>
</body>
</html>"""
