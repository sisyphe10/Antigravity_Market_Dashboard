...
def build_insights(by, dates, i, wics, watch_codes, hist, hist_dates):
    ...
    out['tilt'] = {'reliable': len(nh20) >= TILT_MIN_SAMPLE, 'total': len(nh20),
                   'sectors': len(today_sec), 'items': tilt[:6]}
...
th, td { border: 1px solid var(--aoe-input-border); }
...
.card { height: 100%; max-height: 330px; overflow-y: auto; background: var(--aoe-card2); border: 1px solid var(--aoe-border); border-radius: 10px; padding: 16px; }
.card table { width: 100%; max-width: 560px; margin: 0 auto; }
...
function renderInsights(ins){
  ...
  out += card('신고가',
    '<div class="kpis">' + kpi(t.nh20, '20일', hot)
      + kpi(t.anchor_rate == null ? '-' : t.anchor_rate.toFixed(1) + '%', '종가 안착', false) + '</div>'
  ...
  var f = ins.flow, si = f.strong_in || [], so = f.strong_out || [], chips = '';
  si.forEach(function(x){ chips += '<span class="chip">' + esc(x.name) + ' ' + x.rank + '위</span>'; });
  so.forEach(function(x){ chips += '<span class="chip out">' + esc(x.name) + ' ' + x.prev_rank + '위 이탈</span>'; });
  if (!chips) chips = '<span class="chip out">없음</span>';
  ...
  var th = '<table><thead><tr><th>업종</th><th>오늘</th><th>비중</th><th>직전5일 비중</th></tr></thead><tbody>';
  if (!ti.items.length) th += '<tr><td colspan="4">신고가 없음</td></tr>';
  ...
  var r = ins.radar, rh = '<table><thead><tr><th>종목</th><th>등장</th></tr></thead><tbody>';
  if (!r.items.length) rh += '<tr><td colspan="2">해당 없음</td></tr>';
  r.items.forEach(function(x){
    ...
    var low = x.types.indexOf('low20') >= 0;
    rh += '<tr><td class="' + (low ? 'on' : '') + '">' + esc(x.name) + '</td>'
        + '<td class="' + (low ? 'on' : '') + '">' + esc(tags.join(', ')) + '</td></tr>';
  });
  out += card('보유·관심 레이더',
