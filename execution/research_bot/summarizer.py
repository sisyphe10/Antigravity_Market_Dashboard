...
5. 기본 톤은 간결하게 (불릿 3~5개). 단, '엄중' 코멘트가 있는 항목은 불릿 8~12개 이상으로 상세하게 분석하고, 숫자/데이터/맥락/임팩트를 빠짐없이 포함
6. 본문 불릿 포인트에는 URL을 넣지 말 것
7. 첨부된 이미지(스크린샷, 차트 등)의 내용도 분석하여 요약에 반영
8. 마크다운 볼드(**) 사용하지 말 것. 제목은 ## 또는 ### 헤딩만 사용. 단, 각 불릿 포인트에서 특히 중요한 키워드나 수치는 ==하이라이트== 표시 (예: "- 메모리 수출 ==전년 대비 40% 증가==")
9. '엄중' 태그가 붙은 토픽의 제목 앞에 [엄중] 표시 추가
11. '엄중' 태그가 붙은 메시지에 첨부된 이미지는 관련 내용 바로 아래에 [IMG:번호] 형식으로 삽입 위치를 표시 (예: [IMG:3]). 한 줄에 [IMG:번호]만 단독으로 작성
10. 요약 제일 처음에 "## 출처" 섹션을 만들고, 노트에 포함된 링크를 아래 형식의 표로 정리. 링크가 없으면 출처 섹션 자체를 생략:
| # | 제목 | URL |
| 1 | 기사/페이지 제목 | https://... |
...
엄중이미지: 3, 5 ('엄중' 태그가 붙은 메시지에 첨부된 이미지 번호, 없으면 "없음")
