import os
import base64
import anthropic

def _encode_image(file_path):
    """이미지 파일을 base64로 인코딩"""
    try:
        with open(file_path, 'rb') as f:
            data = base64.standard_b64encode(f.read()).decode('utf-8')
        ext = os.path.splitext(file_path)[1].lower()
        media_types = {'.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.png': 'image/png', '.gif': 'image/gif', '.webp': 'image/webp'}
        return data, media_types.get(ext, 'image/jpeg')
    except:
        return None, None


def summarize_daily_notes(messages, date_str):
    """Claude API로 하루치 리서치 노트를 주제별로 정리/요약 (이미지 분석 포함)"""
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        raise RuntimeError("ANTHROPIC_API_KEY not set")

    # 메시지들을 Claude API content 블록으로 조합
    content_blocks = []
    notes_parts = []

    for i, msg in enumerate(messages, 1):
        parts = [f"[{i}] ({msg['timestamp'][:16]})"]
        if msg.get('forward_source'):
            parts.append(f"[전달: {msg['forward_source']}]")
        if msg.get('text_content'):
            parts.append(msg['text_content'])
        if msg.get('url'):
            parts.append(f"링크: {msg['url']}")
        if msg.get('article_content'):
            parts.append(f"[기사 본문]\n{msg['article_content']}")
        notes_parts.append('\n'.join(parts))

        # 이미지가 있으면 Vision용 블록 추가 (이미지 파일만, PDF 등 제외)
        img_exts = ('.jpg', '.jpeg', '.png', '.gif', '.webp')
        if msg.get('media_path') and msg['message_type'] in ('photo', 'document') and msg['media_path'].lower().endswith(img_exts):
            img_data, media_type = _encode_image(msg['media_path'])
            if img_data:
                notes_parts.append(f"[이미지 {i} 첨부 - 아래 참조]")
                content_blocks.append({
                    "type": "text",
                    "text": f"\n--- 이미지 {i} (메시지 [{i}]의 첨부) ---"
                })
                content_blocks.append({
                    "type": "image",
                    "source": {"type": "base64", "media_type": media_type, "data": img_data}
                })

    notes_text = '\n\n---\n\n'.join(notes_parts)

    # 프롬프트 텍스트
    prompt = f"""너는 월스트리트 IB의 최고 유능한 애널리스트 RA다. 꼼꼼하고 재능이 넘치며 열정과 야망이 넘친다. 다음은 {date_str}에 수집한 리서치 노트이다. 이 노트들을 주제별로 분류하고, 핵심 내용을 정리해라.

## 작성 규칙
1. 한글로 작성. 영문 내용은 자동 번역하여 한글로 정리
2. 관련된 노트들을 주제(토픽)별로 묶기
3. 각 토픽의 요약은 불릿 포인트 활용. 표가 적합한 경우 표로 정리
4. 각 토픽에서 언급된 종목명 + 관련될 수 있는 종목명을 불릿 포인트에 함께 정리 (예: "- 삼성전자: 반도체 수출 호조 전망")
5. 모든 토픽을 불릿 8~12개 이상으로 상세하게 분석하고, 숫자/데이터/맥락/임팩트를 빠짐없이 포함
6. 단, 원본 메시지가 이미 불릿 포인트 형태이거나 단답식으로 요약되어 있는 경우에는 원문을 그대로 유지 (재요약하지 않음). 줄글 형태의 내용만 요약 정리
7. 본문 불릿 포인트에는 URL을 넣지 말 것
8. 첨부된 이미지(스크린샷, 차트 등)의 내용도 분석하여 요약에 반영
9. 이미지에 제목/헤더가 있는 경우: [IMG:번호] 바로 위에 이미지 캡션으로 제목을 적어줄 것 (예: "[캡션: 2025년 반도체 수출 추이]")
10. 이미지에 텍스트가 많은 경우 (표, 기사 캡처 등): [IMG:번호] 아래에 이미지 내 텍스트를 OCR하여 옮겨 적기. 내용이 길면 핵심만 요약
11. 마크다운 볼드(**) 사용하지 말 것. 제목은 ## 또는 ### 헤딩만 사용. 단, 각 불릿 포인트에서 특히 중요한 키워드나 수치는 ==하이라이트== 표시 (예: "- 메모리 수출 ==전년 대비 40% 증가==")
12. 이미지가 첨부된 메시지는 관련 내용 바로 아래에 [IMG:번호] 형식으로 삽입 위치를 표시 (예: [IMG:3]). 한 줄에 [IMG:번호]만 단독으로 작성
13. 원본 메시지에 '엄중', 'ㅇㅈ', '중요', 'ㅈㅇ' 등의 표시가 있으면 해당 토픽 제목에 {{RED}} 태그 추가 (예: "## {{RED}}반도체 수급 이슈"). 제목 텍스트에 [중요] 같은 문구는 넣지 말 것
14. 요약 내용 중 특히 중요하다고 판단되는 불릿 포인트는 앞에 {{RED}} 태그 추가 (예: "- {{RED}}DRAM 가격 ==전주 대비 15% 급등==")
15. 토픽의 출처가 특정 증권사/기관 리포트 또는 기사인 경우, 토픽 제목 뒤에 출처를 괄호로 표기. 증권사 약어: Goldman Sachs→GS, JP Morgan→JPM, Morgan Stanley→MS, 삼성증권→삼성, NH투자증권→NH, 한국투자증권→한투, 미래에셋증권→미래에셋, KB증권→KB, 신한투자증권→신한, 키움증권→키움, 대신증권→대신, 하나증권→하나, BofA→BofA, UBS→UBS, HSBC→HSBC, Citi→Citi, Barclays→Barclays. 기사인 경우 매체명 표기 (예: "## 해상풍력 정책 변화 (매일경제)", "## AI 투자 확대 (Bloomberg)")
16. 요약 제일 처음에 "## 출처" 섹션을 만들고, 노트에 포함된 링크를 아래 형식의 표로 정리. 링크가 없으면 출처 섹션 자체를 생략:
| # | 제목 | URL |
| 1 | 기사/페이지 제목 | https://... |
출처 표 다음에 구분선(---) 넣고, 이후 토픽별 요약 작성

## 출력 마지막에 반드시 아래 형식으로 메타데이터 작성 (이 형식 정확히 지킬 것)
토픽: 주요 토픽1, 토픽2 (쉼표 구분, 최대 3개)
종목: 삼성전자, SK하이닉스, 알지노믹스, 하이브 (본문에서 언급된 모든 개별 종목명을 쉼표 구분으로 나열. 섹터/업종명이 아닌 개별 회사/종목명만. 없으면 "없음")
이미지: 3, 5 (이미지가 첨부된 메시지 번호, 없으면 "없음")

## 수집된 노트 ({len(messages)}건)
{notes_text}"""

    # content 블록 조합: 프롬프트 텍스트 먼저, 이미지 블록 뒤에
    all_content = [{"type": "text", "text": prompt}] + content_blocks

    client = anthropic.Anthropic(api_key=api_key)

    # 재시도 로직 (529 Overloaded 대응)
    import time
    for attempt in range(3):
        try:
            response = client.messages.create(
                model="claude-sonnet-4-20250514",
                max_tokens=4096,
                messages=[{"role": "user", "content": all_content}]
            )
            return response.content[0].text
        except Exception as e:
            if '529' in str(e) or 'overloaded' in str(e).lower():
                if attempt < 2:
                    time.sleep(60)  # 1분 대기 후 재시도
                    continue
            raise


def extract_topics(summary_text):
    """요약 텍스트에서 토픽 키워드 추출"""
    topics = []
    for line in summary_text.split('\n'):
        if line.strip().startswith('토픽:') or line.strip().startswith('토픽 :'):
            raw = line.split(':', 1)[1].strip()
            topics = [t.strip() for t in raw.split(',') if t.strip()]
            break
    return topics[:5]


def extract_stocks(summary_text):
    """요약 텍스트에서 종목명 추출"""
    stocks = []
    for line in summary_text.split('\n'):
        if line.strip().startswith('종목:') or line.strip().startswith('종목 :'):
            raw = line.split(':', 1)[1].strip()
            if raw != '없음':
                stocks = [s.strip() for s in raw.split(',') if s.strip()]
            break
    return stocks


def extract_image_indices(summary_text):
    """요약 텍스트에서 이미지 번호 추출"""
    for line in summary_text.split('\n'):
        stripped = line.strip()
        if stripped.startswith('이미지:') or stripped.startswith('이미지 :'):
            raw = stripped.split(':', 1)[1].strip()
            if raw == '없음':
                return []
            try:
                return [int(x.strip()) for x in raw.split(',') if x.strip().isdigit()]
            except:
                return []
    return []
