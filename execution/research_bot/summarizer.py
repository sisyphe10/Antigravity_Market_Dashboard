...
13. 토픽의 출처가 특정 증권사/기관 리포트 또는 기사인 경우, 토픽 제목 뒤에 출처를 괄호로 표기. 증권사 약어: Goldman Sachs→GS, JP Morgan→JPM, Morgan Stanley→MS, 삼성증권→삼성, NH투자증권→NH, 한국투자증권→한투, 미래에셋증권→미래에셋, KB증권→KB, 신한투자증권→신한, 키움증권→키움, 대신증권→대신, 하나증권→하나, BofA→BofA, UBS→UBS, HSBC→HSBC, Citi→Citi, Barclays→Barclays. 기사인 경우 매체명 표기 (예: "## 해상풍력 정책 변화 (매일경제)", "## AI 투자 확대 (Bloomberg)")
14. 요약 제일 처음에 "## 출처" 섹션을 만들고, 노트에 포함된 링크를 아래 형식의 표로 정리. 링크가 없으면 출처 섹션 자체를 생략:
| # | 제목 | URL |
| 1 | 기사/페이지 제목 | https://... |
