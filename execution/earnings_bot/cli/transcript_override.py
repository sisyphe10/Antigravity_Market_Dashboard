...
def dismiss(ticker: str) -> None:
    """미종결(success/gave_up 외 전체) 상태의 해당 ticker 잡을 'gave_up'으로 종결.
    ...
    try:
        rows = conn.execute(
            ...
            WHERE UPPER(j.ticker) = ?
              AND j.last_status NOT IN ('success', 'gave_up')
            """,
            (ticker.upper(),),
        ).fetchall()
        if not rows:
            print(f"{ticker}: 미종결 잡 없음 "
                  f"(이미 종결됐거나 미존재)")
            return
        conn.execute(
            """
            ...
            WHERE UPPER(ticker) = ?
              AND last_status NOT IN ('success', 'gave_up')
            """,
            (ticker.upper(),),
        )
        conn.commit()
...
def main() -> None:
    ...
    p.add_argument('--list-needs-review', action='store_true',
                   help='미해결 큐 (success/gave_up 외 전체) 출력')
