...
def build_prepared_messages(prepared_remarks: str) -> list[dict]:
    ...
    user_content = f"""다음은 미국 상장사 분기 실적 컨퍼런스콜의 **Prepared Remarks (경영진 발표)** 부분입니다. 시스템 프롬프트의 규칙대로 한국어로 풀 번역하세요. 출력 시작은 `## 경영진 발표` 헤더로 시작하세요. Q&A 섹션은 별도 호출로 처리되니 이번 출력에 포함하지 마세요.
