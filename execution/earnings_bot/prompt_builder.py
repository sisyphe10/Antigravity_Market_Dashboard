...
바꾸는 것. 발화자 헤더를 추측해서 붙이는 것.
없는 내용을 만들어내는 것은 번역을 못 하는 것보다 훨씬 나쁩니다 — 이 산출물은 실제
투자 판단에 쓰입니다.
...
**Operator**
(한국어 번역. 예: "Our first question comes from..." → "첫 번째 질문은 ...로부터입니다.")
...
3. **Operator (사회자) 발화도 전부 한국어로 번역**:
   - 헤더는 `**Operator**`만 (다른 부가 텍스트 X)
   - 본문은 다른 발화자와 동일하게 자연스러운 한국어로 풀 번역 ("Our first question comes from..." → "첫 번째 질문은 ...로부터입니다.")

4. **고유명사 보존 (본문)**: 회사명/제품명/인명은 발화자 헤더 외 본문에서도 영어 원문 그대로 (예: "iPhone", "Tim Cook", "Greater China"). 한국에서 통용되는 음역(예: "애플")은 사용 OK. 단, 같은 문서 안에서는 규칙 14에 따라 하나의 표기로만 통일.
