...
UNIVERSE_FILE = os.path.join(BASE, 'universe_tickers.csv')

# 업종은 Universe(universe_tickers.csv '섹터', WICS 산업그룹 스타일)로 통일 (2026-07-06).
# Universe 미편입 보유 종목만 동일 체계 라벨로 수동 지정 — CSV에 추가되면 CSV가 우선.
SECTOR_OVERRIDES = {
    '000150': '자본재',              # 두산
    '006730': '호텔,레스토랑,레저등',  # 서부T&D
    '032830': '보험',                # 삼성생명
    '402340': '다각화된금융',         # SK스퀘어
    '439260': '자본재',              # 대한조선
    '476830': '제약과생물공학',       # 알지노믹스
    '491000': '건강관리장비와서비스',  # 리브스메드
}
...
def main():
    ...
    # --- 업종 매핑: Universe 섹터 단일 체계 (universe_tickers.csv → SECTOR_OVERRIDES → '기타'+경고) ---
    sector_map = {}
    with open(UNIVERSE_FILE, encoding='utf-8-sig') as f:
        for row in csv.DictReader(f):
            u_code = row.get('티커', '').split(':')[-1].strip()
            u_sec = (row.get('섹터') or '').strip()
            if u_code and u_sec:
                sector_map[u_code] = u_sec
    for c, s in SECTOR_OVERRIDES.items():
        sector_map.setdefault(c, s)
    missing_sec = sorted(c for c in df_w['코드'].unique() if c not in sector_map)
    if missing_sec:
        print(f"   ! 경고: Universe 섹터 미매핑 {len(missing_sec)}종목 → '기타' 처리: "
              f"{[(c, name_map.get(c, '?')) for c in missing_sec]}")
