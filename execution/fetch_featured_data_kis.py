...
범위: 랭킹 6종 + 신고가 3종(newhigh_20d/120d/52w, rank:0)을 한 파일에 누적(KRX 포맷 동일).
출력: featured_data.json (기존 KRX 산출물 drop-in 대체 = 대시보드 직접 소비).
     + newhigh_20d.json (텔레그램 봇용 20일 신고가 상세, is_52w 플래그 포함).
"""
import sys
import os
import json
...
NEWHIGH_DAYS = 20
NEWHIGH_120D_DAYS = 120                          # 120거래일 신고가(대시보드 섹션용)
...
def compute_newhigh_20d(master, prices, date_disp, now_iso):
    ...
    out20 = []
    nh_records = []        # featured_data.json용 신고가 3종(20d/120d/52w) rank:0 레코드
    for code, m in master.items():
        p = prices.get(code)
        ...
        # 대시보드(featured.html) 신고가 섹션용: 달성 타입별 rank:0 레코드 (KRX 포맷 동일)
        for label, achieved in (('newhigh_20d', is_20),
                                ('newhigh_120d', is_120),
                                ('newhigh_52w', is_52w)):
            if achieved:
                nh_records.append({
...
def main():
    ...
    # 신고가: 텔레그램용 20일 상세(newhigh_20d.json) + 대시보드용 3종 featured 레코드
    newhigh, nh_records = compute_newhigh_20d(master, prices, date_disp, now.isoformat())
    records += nh_records                                   # featured_data.json 신고가 섹션
