...
- ★ 한 메시지에 여러 카테고리(테니스 스트로크 여러 개, 또는 여러 운동 종목)가 섞여 있으면 **카테고리별로 분리**해 entries에 각각 한 항목으로 넣는다. 한 종류만 있으면 entries 길이는 1.
  예: "포핸드는 손목 고정, 백핸드는 체중 이동, 발리는 짧게 펀치" → 3개 항목(포핸드/백핸드/포발리 또는 백발리).
...
- 각 항목 필드:
  ...
  · 카테고리: 유형이 "테니스"이면 반드시 다음 7개 중 정확히 하나: {cats}
    슬라이스(포핸드든 백핸드든)는 "슬라이스". 발리는 포/백 구분해 "포발리"/"백발리". 어디에도 안 맞거나 혼합이면 "공통".
...
def _coerce_tennis_cat(c: str):
    ...
    if '슬라이스' in c:
        return '슬라이스'
    if '발리' in c:
        return '포발리' if '포' in c else ('백발리' if '백' in c else '공통')
    if '서브' in c:
        return '서브'
    if '포핸드' in c:
        return '포핸드'
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• 테니스 카테고리: 포핸드·백핸드·슬라이스·포발리·백발리·서브·공통\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('pms:'):
        if data == 'pms:x':
            await q.edit_message_text("취소했어요. 필요할 때 다시 /pms")
            return
        st = _load_pms_state()
        if st.get('corrupt'):
            await q.edit_message_text("⚠️ PMS 상태파일이 손상돼 있어요. 관리자 확인이 필요해요.")
            return
        if st.get('active') and _pms_active_summary(st):
            await q.edit_message_text(_pms_active_summary(st), parse_mode='HTML')
            return  # 버튼 두 번 누름/동시 발동 레이스 가드
        now = datetime.datetime.now(tz=KST)
        anchor = None
        if data != 'pms:s:next':
            try:
                _, _, off, h = data.split(':')
                day = now.date() - datetime.timedelta(days=int(off))
                anchor = datetime.datetime(day.year, day.month, day.day, int(h), tzinfo=KST)
            except (ValueError, TypeError):
                await q.edit_message_text("잘못된 선택이에요. 다시 /pms 를 눌러주세요.")
                return
        jq_missing = context.application.job_queue is None
        await q.edit_message_text(_pms_activate(q.message.chat_id, now, anchor, jq_missing),
                                  parse_mode='HTML')
        return
...
def _pms_parse_anchor(arg_str: str, now: datetime.datetime):
    """시작 슬롯 인자 → anchor datetime(KST) 또는 None.
    'YYYY-MM-DD-HH' / 'YYYY-MM-DD HH' (HH는 08/16/22, ':00' 허용) / 'HH'(오늘)."""
    s = (arg_str or '').strip().replace('시', '')
    m = re.match(r'^(\d{4})-(\d{1,2})-(\d{1,2})[- ](\d{1,2})(?::\d{2})?$', s)
    if m:
        y, mo, d, h = map(int, m.groups())
        if h not in PMS_SLOT_HOURS:
            return None
        try:
            return datetime.datetime(y, mo, d, h, tzinfo=KST)
        except ValueError:
            return None
    h = _pms_parse_anchor_hour(s)
    if h is not None:
        return datetime.datetime(now.year, now.month, now.day, h, tzinfo=KST)
    return None
...
def _pms_active_summary(st: dict) -> str:
    """활성 캠페인 안내 텍스트 (멱등 재발동·버튼 레이스 공용)."""
    ...
    return (f"💊 이미 PMS 케어가 진행 중이에요 (남은 알림 {len(remaining)}회).\n"
            f"▸ 다음 알림: {_pms_fmt_dt(nxt)}\n"
            f"▸ 마지막 알림: {_pms_fmt_dt(end)}\n"
            f"▸ 새로 시작하려면 /pms off 후 /pms")
...
async def cmd_pms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if st.get('active'):
        summary = _pms_active_summary(st)  # 활성 중 재발동은 멱등 — 남은 일정만 안내
        if summary:
