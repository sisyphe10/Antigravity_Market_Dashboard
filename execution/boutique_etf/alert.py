...
def _cells(r):
    ...
    ★구분은 신규(New) / 증가(+) / 감소(-) / 편출(X) — 기호만 두면 눈에 안 띈다는
      지적으로 단어를 붙였다(2026-08-05, 한글 통일 2026-08-11). 그래서 특수 마이너스 기호는 불필요.
    ★비중 화살표 좌우에 공백: 29.7 → 25.1%
    """
    kind = r['kind']
    if kind == 'in':
        gubun, w = '신규(New)', '+%s%%' % _fmt_w(r.get('w_cur'))       # 신규 편입
    elif kind == 'out':
        gubun, w = '편출(X)', '-%s%%' % _fmt_w(r.get('w_prev'))        # 편출
    else:
